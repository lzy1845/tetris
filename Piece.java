...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i++) {
			TPoint p = body[i];
			TPoint newP = new TPoint(this.height-1-p.y, p.x);
			newPoints[i] = newP;
		}
		return new Piece(newPoints); // YOUR CODE HERE
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		// YOUR CODE HERE
		HashSet<TPoint> setOne = new HashSet<TPoint>(Arrays.asList(this.body));
		HashSet<TPoint> setTwo = new HashSet<TPoint>(Arrays.asList(other.body));
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece newPiece;
		while (true) {
			newPiece = current.computeNextRotation();
			if (root.equals(newPiece)) {
				current.next = root;
				break;
			} else {
